...
class EarthNetScore(Metric):
    def __init__(self, dist_sync_on_step=False, compute_on_step=False, use_multiprocessing=False):
    ...
    def update(self, preds, targs):
        ...
        if not isinstance(preds, np.ndarray):
            preds = copy.deepcopy(preds).cpu().numpy()
            masks = copy.deepcopy(targs["dynamic_mask"][0]).cpu().numpy()[:,-preds.shape[1]:,...]
            targs = copy.deepcopy(targs["dynamic"][0]).cpu().numpy()[:,-preds.shape[1]:,...]
        else:
            preds = copy.deepcopy(preds)
            masks = copy.deepcopy(targs["dynamic_mask"])[:,-preds.shape[1]:,...]
            targs = copy.deepcopy(targs["dynamic"])[:,-preds.shape[1]:,...]
        ...
        rows = [[cubenames[i], preds[i, ...], targs[i, ...], masks[i, ...], ndvi_preds[i, ...], ndvi_targs[i, ...],
                 ndvi_masks[i, ...]] for i in range(preds.shape[0])]

        if self.use_multiprocessing:
            with multiprocessing.Pool(preds.shape[0]) as p:
                self.data += list(p.map(parallel_calc, rows))
        else:
            self.data+= [parallel_calc(row) for row in rows]
    ...
    def __get_best_sample(self, samples: Sequence[dict]) -> dict:
        ...
        """
        ens = np.array(
            [self.__harmonic_mean([sample["MAD"], sample["OLS"], sample["EMD"], sample["SSIM"]]) for sample in samples],
            dtype=np.float64)
