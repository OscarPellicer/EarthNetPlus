...
class ConvLSTMCell(nn.Module):
    ...
    def __init__(self, input_size, input_dim, hidden_dim, kernel_size, bias, use_bn, convs_per_cell):
        """
        ...
        convs_per_cell: int
            Number of convolutions per cell. ReLUs in-between
        """
        ...
        if convs_per_cell == 1:
            ...
            for i in range(convs_per_cell):
                ...
                modules.append( nn.Conv2d(  in_channels=in_channels,
                                            ...
                                            padding=self.padding,
                                            bias=self.bias) )
                if i != convs_per_cell - 1:
                    modules.append(nn.SELU(inplace=False))            

            if not use_bn:
                self.conv = nn.Sequential(*modules)
            else:
                self.conv = nn.Sequential([*modules, nn.BatchNorm2d(out_channels)])
    ...
    def __init__(self, input_size, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        for i in range(0, self.num_layers):
            ...
            cell_list.append(ConvLSTMCell(input_size=(self.height, self.width),
                                          ...
                                          bias=self.bias,
                                          use_bn=self.use_bn,
                                          convs_per_cell=self.convs_per_cell))
